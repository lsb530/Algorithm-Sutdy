...
from circular_double_list import DoubleLinkedList
...
while True:
    ...
    elif menu == Menu.멤버십판단:  # 멤버십을 판단(값이 있는지 없는지)
        print('그 값의 데이터는 포함되어'
              + (' 있습니다.' if int(input('판단할 값을 입력하세요 : ')) in lst else ' 있지 않습니다.'))

    elif menu == Menu.모든노드출력:  # 노드를 출력
        lst.print()

    elif menu == Menu.모든노드역순출력:  # 모든 노드를 역순으로 출력
        lst.print_reverse()

    elif menu == Menu.모든노드스캔:  # 모든 노드를 스캔(출력)
        for e in lst:
            print(e)

    elif menu == Menu.모든노드역순스캔:  # 모든 노드를 역순으로 스캔(출력)
